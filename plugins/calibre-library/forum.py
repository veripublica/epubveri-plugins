#!/usr/bin/env python3
# epubveri library for calibre — stage the MobileRead attachment set
...
"""Stage the two files to attach to the first post of MobileRead 375207.

    python3 plugins/calibre-library/forum.py
        -> dist/calibre-library/forum/epubveri_calibre_library_vX.Y.Z.zip
        -> dist/calibre-library/forum/SHA256SUMS.txt
...
carries the same bytes and the same checksum file as the release. Downloading
what GitHub actually serves is also the only way to catch the two mistakes
that matter here — attaching a build of a tree that was never released, and
attaching a version whose release does not exist yet.

**How calibre's index reads a thread** (measured 2026-09-17 against
`setup/plugins_mirror.py` and the live `plugins.json.bz2`): it scrapes one
index post for thread links, takes the **first `.zip` attachment** on each
thread page, and reads the version out of the plugin class in `__init__.py`
with an AST parse. Three consequences:

- the **filename reaches nobody** — calibre stores the download as
  `375207.zip` — so a versioned name is free, and `SHA256SUMS.txt` can name
...
TAG_PREFIX = "calibre-library-v"
ASSET = "epubveri_calibre_library_v%s.zip"
SUMS = "SHA256SUMS.txt"
...
REQUIRED_AT_ROOT = ("__init__.py", "plugin-import-name-epubveri_library.txt")
...
if __name__ == "__main__":
    ...
    print("epubveri library %s (%s) — verified against the release\n" % (version, tag))
